...
const allOrders = async (req, res, next) => {
  try {
    const orders = await Order.find({}).populate("userId").populate("courierId");
...
const orderById = async (req, res, next) => {
  try {
    ...
    const detailOrder = await Order.findById(id).populate("userId").populate("courierId");
...
const OrderId = async (req, res, next) => {
  try {
    ...
    const detailOrder = await Order.findOne({ orderId: id }).populate("userId").populate("courierId");
    res.status(200).send(detailOrder);
  } catch (err) {
    next(err);
  }
...
const allCourierOrders = async (req, res, next) => {
  try {
    ...
    const data = await Order.find({ courierId: courierData }).populate("userId");
...
const orderByCourier = async (req, res, next) => {
  try {
    ...
    const orders = await Order.find(obj).populate("userId").populate("courierId");
    res.status(200).send(orders);
  } catch (err) {
    next(err);
  }
...
const allOrdersByState = async (req, res, next) => {
  try {
    const { state } = req.body;
    const orders = await Order.find({ actualState: state }).populate("userId").populate("courierId");
    res.status(200).send(orders);
  } catch (err) {
    next(err);
  }
...
const orderFilterEcommerce = async (req, res, next) => {
  ...
  try {
    const orders = await Order.find({ $and: [primeraParte, segundaParte] })
      .populate("userId")
      .populate("courierId");
    res.status(200).send(orders);
  } catch (error) {
...
const orderFiltercourier = async (req, res, next) => {
  ...
  try {
    const orders = await Order.find({
      $and: [
        { "stateHistory.0.date": { $gte: new Date(fechaDesde) }, courierId: courierId },
...
const orderFilterMessenger = async (req, res, next) => {
  ...
  try {
    const orders = await Order.find({
      $and: [
        { "stateHistory.0.date": { $gte: new Date(fechaDesde) }, userId: id },
